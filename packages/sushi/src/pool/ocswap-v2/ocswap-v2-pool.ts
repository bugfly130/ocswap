...
import { computeOcSwapV2PoolAddress } from './compute-ocswap-v2-pool-address.js'
import { SerializedOcSwapV2Pool, ocSwapV2PoolSchema } from './zod.js'

export class OcSwapV2Pool {
  ...
  public static getAddress(tokenA: Token, tokenB: Token): string {
    if (!isOcSwapV2ChainId(tokenA.chainId)) {
      throw new Error(
        `ChainId Error: OcSwapV2 is not available on ${
          chainName[tokenA.chainId]
        }`,
      )
    }

    return computeOcSwapV2PoolAddress({
      factoryAddress: OCSWAP_V2_FACTORY_ADDRESS[tokenA.chainId],
      tokenA,
      tokenB,
    })
  }
  ...
  public constructor(amountA: Amount<Token>, amountB: Amount<Token>) {
    ...
    this.liquidityToken = new Token({
      ...
      address: OcSwapV2Pool.getAddress(
        amounts[0].currency,
        amounts[1].currency,
      ),
      decimals: 18,
  ...
  public serialize(): SerializedOcSwapV2Pool {
    return ocSwapV2PoolSchema.parse({
      reserve0: this.tokenAmounts[0].serialize(),
      reserve1: this.tokenAmounts[1].serialize(),
    })
  }

  public static deserialize(pair: SerializedOcSwapV2Pool): OcSwapV2Pool {
    return new OcSwapV2Pool(
